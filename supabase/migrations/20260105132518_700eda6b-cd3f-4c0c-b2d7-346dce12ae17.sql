...
-- Fix get_subscription_info function to properly handle NULL when no rows found
...
BEGIN
  ...
  -- Get matches sent today (with explicit COALESCE after SELECT)
  SELECT COALESCE(SUM(matches_sent), 0) INTO v_matches_sent
  FROM daily_limits
  WHERE user_id = p_user_id AND reset_date = v_today;

  -- Get messages sent this month (with explicit COALESCE after SELECT)
