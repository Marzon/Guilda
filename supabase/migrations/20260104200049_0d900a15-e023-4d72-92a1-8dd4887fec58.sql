-- Fix get_subscription_info function to use correct column name
-- The subscriptions table uses 'member_status' not 'status'
-- And values are 'ACTIVE', 'ENROLLED' not lowercase 'active'
...
BEGIN
  -- Get active subscription tier (fix: use member_status instead of status)
