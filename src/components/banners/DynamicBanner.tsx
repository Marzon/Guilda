...
import { useFilteredBanners, type CoreBanner } from "@/hooks/useCoreAppBanners";
...
export function DynamicBanner({ type, maxBanners = 1 }: DynamicBannerProps) {
   const { banners, isLoading, dismissBanner } = useFilteredBanners(type);
  ...
  const handleClick = (link?: string | null) => {
    ...
    // Se link começa com /, redirecionar para Core App preservando query params
    if (link.startsWith('/')) {
      window.location.href = buildCoreAppUrl(CORE_APP_URL, link);
    ...
    <>
       {displayBanners.map(banner => (
        <BannerRenderer
          key={banner.id}
          banner={banner}
           onDismiss={() => dismissBanner(banner.slug)}
           onClick={handleClick}
        />
      ))}
    </>
